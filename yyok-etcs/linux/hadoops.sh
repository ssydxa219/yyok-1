...
####################component 版本定义
    ...
    vvscala="scala-2.11"
    vzk="apache-zookeeper-3.5.7-bin"
...
########################################################################zk
 ...
 # wget https://downloads.apache.org/zookeeper/zookeeper-3.5.7/apache-zookeeper-3.5.7-bin.tar.gz
  ...
  for zk in $zk_hosts;do
    ...
    ssh $zk "rm -rf $local_dir/zookeeper/* && rm -rf $var_dir/zookeeper/* && touch $local_dir/zookeeper/myid && echo $n > $local_dir/zookeeper/myid && cat $local_dir/zookeeper/myid && exit;"
  done
  ...
  for zk in $zk_hosts;do
   ...
   if [ $zk == $basic_host ];then
    ...
    ssh $zk "chmod -R 755 $bin_dir && chmod -R 755 $local_dir && $bin_dir/zookeeper/bin/zkServer.sh status && $bin_dir/zookeeper/bin/zkServer.sh start && exit;"
   done
}
...
#echo "======================================================"
	...
	mkdir -p $local_dir/hadoop/tmp $local_dir/hadoop/name $local_dir/hadoop/data $local_dir/hadoop/journalnode
 ...
 echo "===========================HADOOP $hadoopname_host etc profile add the HADOOP ================="
      ssh $hadoopname_host "chmod -R 755 $hadoop_dir/etc/hadoop/racks.sh && ntpdate time.nist.gov && rm -rf $var_dir/hadoop/* $local_dir/hadoop/* \
 ...
 echo "===========================HADOOP $hadoopnameha_host etc profile add the HADOOP ================="
    ...
    ssh $hadoopnameha_host "ntpdate time.nist.gov && rm -rf $hadoop_dir $var_dir/hadoop/* $local_dir/hadoop/* \
    && ! cat /etc/profile | grep \"HADOOP_HOME\" && echo 'export HADOOP_HOME=$bin_dir/hadoop' >>/etc/profile \
    && ! cat /etc/profile | grep \"HADOOP_HDFS_HOME\" && echo 'export HADOOP_HDFS_HOME=\$HADOOP_HOME' >>/etc/profile \
    && ! cat /etc/profile | grep \"HADOOP_MAPRED_HOME\" && echo 'export HADOOP_MAPRED_HOME=\$HADOOP_HOME' >>/etc/profile \
    && ! cat /etc/profile | grep \"HADOOP_COMMON_HOME\" && echo 'export HADOOP_COMMON_HOME=\$HADOOP_HOME' >>/etc/profile \
    ...
    scp -r $hadoop_dir root@$hadoopnameha_host:$bin_dir;
      for scps in $hadoopdata_hosts;do
       if [ $scps == $basic_host ];then
            ...
            ssh $scps "ntpdate time.nist.gov && rm -rf $hadoop_dir $local_dir/hadoop/* $var_dir/hadoop/* && chmod -R 755 $basic_dir \
                && ! cat /etc/profile | grep \"HADOOP_HOME\" && echo 'export HADOOP_HOME=$bin_dir/hadoop' >>/etc/profile \
    && ! cat /etc/profile | grep \"HADOOP_HDFS_HOME\" && echo 'export HADOOP_HDFS_HOME=\$HADOOP_HOME' >>/etc/profile \
    && ! cat /etc/profile | grep \"HADOOP_MAPRED_HOME\" && echo 'export HADOOP_MAPRED_HOME=\$HADOOP_HOME' >>/etc/profile \
    && ! cat /etc/profile | grep \"HADOOP_COMMON_HOME\" && echo 'export HADOOP_COMMON_HOME=\$HADOOP_HOME' >>/etc/profile \
...
echo "${hbasemasterha_host}" >> $hbase_dir/conf/backup-masters
 for regs in $regionserver_hosts;do
   ...
   # echo "export HBASE_REGIONSERVER_OPTS=\"\$HBASE_REGIONSERVER_OPTS -XX:PermSize=128m -XX:MaxPermSize=128m\"" >>$hbase_dir/conf/hbase-env.sh
    ...
    echo "export HBASE_REGIONSERVER_OPTS=\"\$HBASE_REGIONSERVER_OPTS \$HBASE_JMX_BASE -Xmx20g -Xms20g -Xmn1g -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=70\"" >>$hbase_dir/conf/hbase-env.sh
    ...
    echo "<value>$local_dir/zookeeper</value>" >>$hbase_dir/conf/hbase-site.xml
    echo "</property>" >>$hbase_dir/conf/hbase-site.xml
    echo "<property>" >>$hbase_dir/conf/hbase-site.xml
...
sed -i "32a env.hadoop.conf.dir: $hadoop_dir" $flink_dir/conf/flink-conf.yaml
...
source /etc/profile
cp $src_dir/flink-shaded-hadoop2-2.8.3-1.8.3.jar $flink_dir/lib/
