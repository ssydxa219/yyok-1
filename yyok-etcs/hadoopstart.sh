...
ssh $j "rm -rf /ddhome/local/hadoop/journal/yyok && /ddhome/bin/hadoop/bin/hdfs --daemon $1 journalnode"
done
...
function inithadoop(){
    ...
    exit
    hdfs haadmin -transitionToActive --forcemanual  dda
    hdfs haadmin -getServiceState dda
...
#ssh dda /ddhome/bin/hadoop/sbin/start-all.sh
