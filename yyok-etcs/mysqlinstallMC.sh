...
echo "PATH=\$PATH:/ddhome/bin/mysql/bin" >> /etc/profile
source /etc/profile
#adduser mysql
#passwd root
#groupadd mysql
#tar gz
rm -rf /ddhome/bin/mysql /etc/my.cnf /ddhome/local/mysql/* /ddhome/var/mysql/*
...
chgrp -R mysql /ddhome/bin/mysql
chown -R mysql /ddhome/bin/mysql
chmod -R 755 /ddhome/ 
chmod -R 755 /ddhome/bin/mysql/tmp/
mkdir  -p /ddhome/local/mysql/data /ddhome/bin/mysql/tmp /ddhome/var/mysql/
...
chown -R mysql:mysql /ddhome/local/mysql/data
chown -R mysql:mysql /ddhome/bin/mysql/tmp/
...
echo "datadir = /ddhome/local/mysql/data" >> /etc/my.cnf
echo "# MySQL数据文件存放路径" >> /etc/my.cnf

echo "tmpdir = /ddhome/bin/mysql/tmp/" >> /etc/my.cnf
...
echo "general_log_file = /ddhome/var/mysql/general.log" >> /etc/my.cnf
...
echo "slow_query_log_file = /ddhome/var/mysql/slow.log" >> /etc/my.cnf
...
echo "log_error =/ddhome/var/mysql/error.log" >> /etc/my.cnf
...
echo "log_bin = /ddhome/var/mysql/binlog" >> /etc/my.cnf
echo "# 二进制日志存放路径" >> /etc/my.cnf

echo "log_bin_index = /ddhome/var/mysql/binlog.index" >> /etc/my.cnf
...
echo "relay_log = /ddhome/var/mysql/relaylog" >> /etc/my.cnf
echo "# 中继日志存放路径" >> /etc/my.cnf

echo "relay_log_index = /ddhome/var/mysql/relaylog.index" >> /etc/my.cnf
...
echo "log-error=/ddhome/var/mysql/mariadb.log" >> /etc/my.cnf
echo "pid-file=/ddhome/bin/mysql/run_mariadb.pid" >> /etc/my.cnf
...
cp /ddhome/bin/mysql/support-files/mysql.server /etc/init.d/mysqld
...
sed -i  "45d" /etc/rc.d/init.d/mysqld
sed -i '45d' /etc/init.d/mysqld
...
chgrp -R mysql /ddhome/bin/mysql
chown -R mysql /ddhome/bin/mysql
chgrp -R mysql /ddhome/bin/mysql/tmp
chown -R mysql /ddhome/bin/mysql/tmp
chmod -R 600 /etc/my.cnf
yum install -y perl-Module-Install.noarch
#touch /ddhome/bin/mysql/mysql.pid
rm -rf /ddhome/local/mysql/data/*
chmod -R 755 /ddhome/bin/mysql
chmod -R 755 /ddhome/bin/mysql/tmp
chmod -R 755 /ddhome/local/mysql
chmod -R 755 /ddhome/var/mysql
...
#命令说明
#1.update user set authentication_string=password('root'),password_expired='N' where user='root';
